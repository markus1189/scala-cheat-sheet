...
\begin{center}
  \begin{tikzpicture}
    \node[sup](opt){Option[A]};
    \node[sup, below left of=opt, xshift=-15mm, yshift=-15mm](either){Either[A, B]};
    \node[sup, below right of=opt, xshift=15mm, yshift=-15mm](try){Try[A]};

    \draw [line, <->] (opt.west) to [bend right=30] node{toRight} (either.north);
    \draw [line, <->] (either.east) to (try.west);
    \draw [line, <->] (try.north) to [bend right=30] (opt.east);
    ;
  \end{tikzpicture}
\end{center}
\section{Operators}
